-- ============================================================
-- Sourcing network: include audit firms (account_type = auditor)
-- as companies alongside sellers / service providers.
--
-- Buyers request audits by selecting an audit company (firm),
-- never an individual auditor person (Audit Pro roster is separate).
-- Rows are always company-scoped (public.companies); email prefers
-- the company mailbox over a personal profile address.
-- ============================================================
...
BEGIN
  ...
  SELECT
    c.id,
    -- Company mailbox first (selection / notify target is the firm, not a person)
    COALESCE(NULLIF(LOWER(TRIM(c.email)), ''), NULLIF(LOWER(TRIM(p.email)), '')) AS email,
    COALESCE(NULLIF(TRIM(c.name), ''), 'Company') AS company,
    -- Optional company contact only; not used as the audit assignee identity
...
COMMENT ON FUNCTION public.list_sourcing_network_accounts(INTEGER) IS
  'Authenticated sourcing directory of companies (seller, service_provider, auditor firms). Buyers select audit companies here; individual auditors live in management_audit_directory / Audit Pro, not this list.';
